...
class CRUD(Databases):
    ...
    def readDB(self, name):
        sql = "SELECT * \
            FROM public.equipment_equipmentinuse"  \
            .format(name=name)
        try:
            # SQL 쿼리 실행
            self.cursor.execute(sql)
    ...
    # Update nfc_uid , start_time , equipmnet_id값 업데이트
    def updateNFC(self, nfc_uid, start_time, equipmnet_id):
        sql = "UPDATE public.equipment_equipmentinuse \
            SET nfc_uid='{nfc_uid}' start_time='{start_time}' equipmnet_id='{equipmnet_id}'" \
            .format(nfc_uid=nfc_uid, start_time=start_time, equipmnet_id=equipmnet_id)
        try:
            # SQL 쿼리 실행 및 커밋
            self.cursor.execute(sql)
            self.db.commit()
    ...
    def deleteDB(self, condition):
        sql = "DELETE FROM public.equipment_equipmentinuse WHERE {condition};" \
